    ...
    <>
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-56 flex-col border-r border-white/10 bg-black md:flex">
        <div className="flex items-center gap-2 border-b border-white/10 px-5 py-4">
          <span className="flex h-8 w-8 items-center justify-center bg-amber-500 font-display text-lg font-bold text-black">
            G
          </span>
          <div>
            <p className="font-display text-sm font-bold uppercase tracking-widest">Garage</p>
            <p className="text-[10px] tracking-wider text-zinc-500 uppercase">Panel operativo</p>
          </div>
        </div>
          ...
          return (
            <Link
              ...
              className={`whitespace-nowrap rounded-md px-3 py-1.5 text-xs font-medium ${
                active ? "bg-amber-500/10 text-amber-400" : "text-zinc-400"
              }`}
            >
              {l.icon} {l.label}
            </Link>
